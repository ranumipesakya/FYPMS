...
const SupervisorDashboard: React.FC = () => {
  ...
  const handleOpenOfficeHours = async () => {
    ...
    try {
      const { data } = await axios.post('http://localhost:5001/api/projects/office-hours/open', {
        date
      }, {
        headers: { Authorization: `Bearer ${user?.token}` }
      });
      setScheduleMessage(`Scheduled ${data.created}/${data.totalStudents} slots (${data.skipped} already existed).`);
    } catch (err: any) {
    ...
    <div className="pt-32 px-6 pb-20 max-w-7xl mx-auto min-h-screen text-slate-100 font-inter">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-4 animate-in fade-in slide-in-from-top duration-700">
        <div>
          ...
          {scheduleMessage && <p className="text-xs text-brand-blue font-bold mt-2">{scheduleMessage}</p>}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        ...
        <button
          ...
          className="glass p-8 rounded-[50px] border-2 border-dashed border-white/10 flex flex-col items-center justify-center text-center group cursor-pointer hover:border-brand-blue/40 transition-all min-h-[300px] disabled:opacity-60"
        >
          <div className="w-16 h-16 bg-brand-blue/10 rounded-full flex items-center justify-center text-brand-blue mb-4 group-hover:scale-110 transition-transform">
            {isScheduling ? <Loader2 size={32} className="animate-spin" /> : <CheckCircle2 size={32} />}
          </div>
