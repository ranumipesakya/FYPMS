...
const ProjectSetup: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Mock preservation of data
    setTimeout(() => {
      // In real scenario, this would POST /api/projects/initiate
      setIsLoading(false);
      navigate('/dashboard');
    }, 2000);
  };
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-100 flex items-center justify-center pt-28 pb-20 px-6 font-inter">
      ...
      <div className="max-w-4xl w-full relative z-10">
        <div className="text-center mb-12 animate-in fade-in slide-in-from-top duration-700">
           ...
           <h1 className="text-4xl md:text-5xl font-extrabold font-outfit text-white mb-4 tracking-tight">Initialize Your <span className="text-brand-blue italic">Final Year Journey</span></h1>
        ...
        <div className="glass p-8 md:p-12 rounded-[50px] border border-white/5 shadow-2zl overflow-hidden relative">
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-4">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest block ml-1">Student Full Name</label>
            ...
            {/* Student Number */}
            <div className="space-y-4">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest block ml-1">Student Number</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-brand-blue transition-colors">
                  <Hash size={18} />
                </div>
            ...
            <div className="md:col-span-2 space-y-4">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest block ml-1">Project Title</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-500 group-focus-within:text-brand-blue transition-colors">
                  <Briefcase size={18} />
                </div>
            ...
            <div className="md:col-span-2 space-y-4">
              <label className="text-xs font-black text-slate-500 uppercase tracking-widest block ml-1">Assigned Supervisor</label>
              <div className="relative group">
                ...
                >
                  <option value="" className="bg-[#0f172a] text-slate-500">Select your supervisor...</option>
                  {SUPERVISORS.map((name, idx) => (
                    <option key={idx} value={name} className="bg-[#0f172a] text-white py-2">
                       {name}
                    </option>
                  ))}
                </select>
